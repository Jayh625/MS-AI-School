...
def main() :
    ...
    model.to(device)
    model.load_state_dict(torch.load(f="./result2/20230718_103859/best.pt"))
    ...
    test_dataset = CustomDataset("./data/US_license_plates_dataset/test/", transform=val_transforms)
    test_loader = DataLoader(test_dataset, batch_size=1, shuffle=False)

    model.to(device)
    model.eval()
    correct = 0 
    data_dir = "./data/US_license_plates_dataset/test/"
    dict = {}
    folders = os.listdir(data_dir)
    for i, folder in enumerate(folders) :
        dict[folder] = i
    label_dict = {v: k for k, v in dict.items()}
    os.makedirs("./result2/imgs", exist_ok=True)
    with torch.no_grad() :
        for i, (data, target, path) in tqdm(enumerate(test_loader)) :
            ...
            cv2.imwrite(os.path.join("./result2/imgs", f"output_{str(i).zfill(4)}.png"),img)
