...
from base_gan_model import Generator, Discriminator
...
optimizer_G = optim.AdamW(generator.parameters(), learning_rate)
optimizer_D = optim.AdamW(discriminator.parameters(), learning_rate)
...
for epoch in range(epochs) :
    for i, (real_images, _) in enumerate(dataloader) :
        # 판별자의 그레디언트 0으로 초기화 -> 역전파 단계에서 새로운 그레디언트 계산하기전에 기존값을 제거
        ...
        실제 이미지 데이터를 모델에 입력하기 위해서 형태 조정
            ...
            -> 2차원 -> 1차원 벡터로 변환하여 모델 입력 준비
        """

        # real images discriminator val, loss
        outputs_real = discriminator(real_images) # 실제 이미지 -> 판별자 모델 -> 판별 결과
        loss_real = criterion(outputs_real, real_labels)
        # 실제 이미지 -> 판별자의 출력 결과 -> 실제 이미지에 대한 레이블 간의 손실 계산 (판별자가 얼마나 실제 이미지를 정확하게 판별하는가를 나타내는 값)

        # fake images discriminator val, loss
        noise = torch.randn(batch_size, 100).to(device)
        # 생성자에 입력으로 들어갈 노이즈 벡터 생성 -> 노이즈 벡터는 가짜 이미지 생성하는 데 사용 -> 100 : 노이즈 벡터의 차원
        fake_images = generator(noise) # 노이즈 벡터 -> 생성 모델 -> 가짜 이미지 생성
        fake_images = fake_images.view(-1, 784) # 생성된 가짜 이미지 데이터를 -> 판별기 모델 넣기 위해서 1차원 벡터로 변환 필요
        outputs_fake = discriminator(fake_images.detach()) # 생성된 가짜 이미지 -> 판별 모델 -> 판별 결과 출력
        loss_fake = criterion(outputs_fake, fake_labels) 
        # 가짜 이미지에 대한 판별자 모델 -> 출력결과 -> 가짜 이미지에 대한 레이블 간의 손실 계산 (이 손실은 결국 판별자가 가짜 이미지를 얼마나 정확하게 판별하는가를 나타냅니다)
        
        # all loss (Discriminator)
        loss_D = loss_real + loss_fake # 판별자의 전체 손실 -> 실제 이미지에 대한 손실과 가짜이미지에 대한 손실 합친값
        loss_D.backward()
        optimizer_D.step()

        # generator train
        generator.zero_grad() # 생성자의 그레디언트 0 초기화
        # 생성자는 판별자를 속이는 방향으로 학습되어야 합니다 -> 그래서 생성자만의 그레이디언트를 따로 계산하고 업데이트 진행합니다
