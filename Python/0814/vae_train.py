...
for epoch in range(num_epochs) :
    for i, (images, _) in enumerate(train_loader) :
        ...
        # images : input images
        # 재구성 오차 -> 입력 이미지와 재구성된 이미지 간 차이를 계산하고 배치 크기로 나눠 정규화
        ...
        # mu : 잠재 변수의 평균 
        # logval : 잠재 변수의 로그 분산값
        kl_devergence = -0.5 * torch.sum(1 + logval - mu.pow(2) - logval.exp()) / batch_size

        # all loss
        loss = reconstruction_loss + kl_devergence
        # VAE의 목표는 재구성ㅇ 오차를 최소화 하면서 동시에 KL 다이버전스 줄이는 것입니다
        loss.backward()
        optimizer.step()
