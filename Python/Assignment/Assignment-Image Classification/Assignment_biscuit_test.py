...
def main() :
    ...
    # .pt load 
    model.load_state_dict(torch.load(f="./data/02_best.pt"))
    ...
    test_dataset = CustomDataset("./data/Biscuit Wrappers/val/", transform=val_transforms)
    ...
    model.eval()
    correct = 0 
    data_dir = "./data/Biscuit Wrappers Dataset/"
    dict = {}
    ...
    with torch.no_grad() :
        for i, (data, target, path) in enumerate(test_loader) :
            target_ = target.item()
            ...
            img = cv2.rectangle(img, (0,0), (700,100), (0,0,0), -1)
            ...
            #     exit()
            cv2.imwrite(os.path.join("./result/imgs", f"output_{str(i).zfill(4)}.png"),img)
