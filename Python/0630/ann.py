...
class ANN(nn.Module) :
    def __init__(self, input_size, hidden_size, output_size) :
        ...
        # super(ANN, self) : ANN의 상위 클래스를 호출한다고 명시, 
        #                   상속단계가 2단계 이상일때 의미가 있음
        # super() : 부모 클래스가 한단계밖에 없는 경우 위와 완전히 동일한 역할
        self.fc1 = nn.Linear(input_size, hidden_size)
        # nn.Linear(input_size, hidden_size) : input_size -> 입력받을 크기
        #                                      hidden_size -> 이 층이 출력할 크기
        self.relu = nn.ReLU()
    ...
    def forward(self, x) : 
        ...
        out = self.fc1(x)
        # nn.Linear(input_size, hiddem_size)로 정의된 계층에 x를 통과시킴
        # 입력층 - 은닉층 연산 진행
...
if __name__ == "__main__" :
    ...
    labels = torch.randint(0, output_size, (100, )) # 100크기의 라벨
