...
2. 보기버튼 누르면 입력 정보를 보여준다
3. 입력 정보 창에서는 (나이, 성별, 국가) 입력받은 정보 출력
- 추가적으로 저장 버튼, 닫기 버튼 불러오기 버튼 추가
4. 저장된 정보창에서는 리스트 박스를 이용하여
(중복 방지를 위한 ID 추가 여기서는 Time 함수를 이용)
DB처럼 auto number 없음 저장된 정보를 읽어 오기
- ID / 나이 / 성별 / 국가
"""
import sys
import csv 
...
class InfoWindow(QDialog) :
    ...
    def save_info(self, age, gender,country) :
        ...
        try :
            ...
            QMessageBox.information(self, "Info Saved", "Info Saved")
        except Exception as e :
            QMessageBox.critical(self, "Save Failed", f"{str(e)}")
    ...
    def load_info(self) :
        try :
            ...
            if len(lines) > 0 :
                ...
                QMessageBox.information(self, "Load Failed")

        except Exception as e :
            QMessageBox.critical(self, "Load Failed", f"{str(e)}")
