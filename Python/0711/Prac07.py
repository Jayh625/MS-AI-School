...
from PySide6.QtWidgets import QApplication, QWidget, QVBoxLayout, \
    QGroupBox, QLabel, QLineEdit, QPushButton, QListWidget, QMessageBox
...
class MainWindow(QWidget) :
    def __init__(self):
        ...
        layout2.setContentsMargins(10, 10, 10, 10) # left, top, right, bottom
    ...
    def load_info(self) :
        try :
            with open("data.csv", "r") as f :
                reader = csv.reader(f)
                for row in reader : 
                    data_text = f"ID {row[0]}, age {row[1]}, gender {row[2]}, country {row[3]}"
