...
for file in tqdm(train_files) : 
    ...
    with open(f"./yolo_dataset/train/labels/{file_name}.txt", 'w') as f :
...
for file in tqdm(val_files) : 
    file_name = os.path.basename(file)
    file_name = file_name.split(".")[0]
    img = cv2.imread(f"./dataset/val/{file_name}.png")
    ...
    with open(f"./yolo_dataset/val/labels/{file_name}.txt", 'w') as f :
        for line in lines_list :
            f.write(str(line[0]) + ' ' + str(line[1]) + ' ' + str(line[2]) + ' ' + str(line[3]) + ' ' + str(line[4]) + '\n')