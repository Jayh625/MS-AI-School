    ...
    def find_xml_file(self) :
        ...
        for (path, dir, files) in os.walk(self.xml_folder_path) :
            for filename in files :
                ext = os.path.splitext(filename)[-1] # .xml
                if ext == ".xml" :
            ...
            for img_meta in img_metas :
                try :
                    ...
                    for box_meta in box_metas : 
                        ...
                        change_fore_ocuure_temp = 2 # 겹쳐 있는 포인트
                        os.makedirs("./dataset/train/labels", exist_ok=True)
                        with open(f"./dataset/train/labels/{image_name}", 'a') as f:
                            f.write(
                                f"{label_number} {box_x:.6f} {box_y:.6f} {box_w:.6f} {box_h:.6f} {head_x_temp:.6f} {head_y_temp:.6f} {change_fore_ocuure_temp} {tail_x_temp:.6f} {tail_y_temp:.6f} {change_fore_ocuure_temp}\n" 
                            )
...
annot_dir = "./yolo_keypoint_data/annotations/train"
test = InfraredImageProcessor(annot_dir)
test.process_images()