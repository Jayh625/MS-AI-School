...
def get_model(num_keypoints) :
    ...
    model = torchvision.models.detection.keypointrcnn_resnet50_fpn(pretrained=False, pretrained_backbone=True, 
                                                                   num_keypoints=num_keypoints, num_classes=2, 
...
def visualize(image, bboxes, keypoints) :
    ...
    bboxes_scaled = [[int(coord * scale_factor) for coord in bbox] for bbox in bboxes]
    keypoints_scaled = [[[int(coord[0] * scale_factor), int(coord[1] * scale_factor)] for coord in kps] for kps in keypoints]
    ...
    for kps in keypoints_scaled :
        for idx, kp in enumerate(kps) :
            image_resize = cv2.circle(image_resize.copy(), tuple(kp), 
                                        5, (255,0,0), 5)
...
def main() :
    ...
    keypoint_test_data_path = "./keypoint_dataset/test"
    test_dataset = KeypointDataset(keypoint_test_data_path, transform=None, demo=False)
    test_loader = DataLoader(test_dataset, batch_size=1, shuffle=False, collate_fn=collate_fn)
    
    model = get_model(num_keypoints=2)
    model.load_state_dict(torch.load(f="./keypointsrcnn_weights_20.pth", map_location=device))
    model.to(device)
    model.eval()
    ...
    for images, targets in test_loader :
        ...
        keypoints = []
        for kps in output[0]['keypoints'][high_scores_idxs][post_nms_boxes].detach().cpu() :
            if len(kps) > 0 :
